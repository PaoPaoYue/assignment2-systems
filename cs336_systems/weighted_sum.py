...
def test_weighted_sum():
    ...
    # --- Forward ---
    y_custom = WeightedSumFunc.apply(x, w)
    y_ref = weighted_sum_ref(x, w)
    # 检查 forward 是否接近
    if not torch.allclose(y_custom, y_ref, rtol=1e-4, atol=1e-5):
        print("❌ Forward mismatch")
        print("custom:", y_custom)
        print("ref   :", y_ref)
    else:
    ...
    # 检查梯度
    if torch.allclose(grad_x_custom, grad_x_ref, rtol=1e-4, atol=1e-5):
        print("✅ grad_x matches reference")
    else:
        print("❌ grad_x mismatch")

    if torch.allclose(grad_w_custom, grad_w_ref, rtol=1e-4, atol=1e-5):
        print("✅ grad_w matches reference")
    else:
        print("❌ grad_w mismatch")
